...
class RepoAuditCheckpointStore(context: Context) {
    ...
    @Synchronized
    fun save(run: RepoAuditRun) {
        GZIPOutputStream(file.outputStream()).use { it.write(encode(run).toString().toByteArray(Charsets.UTF_8)) }
    }

    @Synchronized
    fun load(): RepoAuditRun? {
    ...
    private fun encode(run: RepoAuditRun): JSONObject = JSONObject()
        .put("repo", run.repoFullName)
        .put("ref", run.ref)
        .put("commit", run.commitSha)
        .put("stage", run.stage.name)
        .put("required", run.requiredFiles)
        .put("excluded", run.excludedFiles)
        .put("chairman", run.chairmanModel)
        .put("verification", run.verificationMemo)
        .put("final", run.finalReport)
        .put("started", run.startedAt)
        .put("finished", run.finishedAt ?: JSONObject.NULL)
        ...
        .put("aggregate", JSONArray().apply {
            run.aggregate.forEach { put(JSONObject().put("model", it.model).put("avg", it.averageRank).put("votes", it.votes)) }
        })
        .put("excluded_manifest", JSONArray().apply {
            run.excludedManifest.forEach { f ->
                put(JSONObject()
                    .put("path", f.path)
                    .put("sha", f.sha)
                    .put("size", f.size)
                    .put("category", f.category)
                    .put("reason", f.exclusionReason ?: "excluded"))
            }
        })

    private fun encodeAudit(a: ModelRepoAudit): JSONObject = JSONObject()
        .put("model", a.model)
        .put("report", a.report)
        .put("complete", a.complete)
        .put("error", a.error ?: JSONObject.NULL)
        .put("batches", JSONArray(a.batchReports))
        .put("coverage", JSONArray().apply {
            a.coverage.forEach { c ->
                put(JSONObject().put("path", c.path).put("model", c.model).put("covered", c.covered)
                    .put("batch", c.batchIndex).put("error", c.error ?: JSONObject.NULL))
            }
        })

    private fun encodeReview(r: RankingReview): JSONObject = JSONObject()
        .put("model", r.model)
        .put("text", r.text)
    ...
    private fun decode(o: JSONObject): RepoAuditRun {
        ...
        val aggregate = mutableListOf<AggregateRank>()
        val ag = o.optJSONArray("aggregate") ?: JSONArray()
        for (i in 0 until ag.length()) {
            val x = ag.getJSONObject(i)
            aggregate += AggregateRank(x.optString("model"), x.optDouble("avg"), x.optInt("votes"))
        }

        val exclusions = mutableListOf<RepoFile>()
        val ex = o.optJSONArray("excluded_manifest") ?: JSONArray()
        for (i in 0 until ex.length()) {
            val f = ex.getJSONObject(i)
            exclusions += RepoFile(
                ...
                binaryOrExcluded = true,
                exclusionReason = f.optString("reason", "excluded")
            )
        }
        ...
        return RepoAuditRun(
            ...
            ref = o.optString("ref"),
            commitSha = o.optString("commit"),
            stage = runCatching { RepoAuditStage.valueOf(o.optString("stage")) }.getOrDefault(RepoAuditStage.IDLE),
            requiredFiles = o.optInt("required"),
            excludedFiles = o.optInt("excluded"),
            modelAudits = audits,
            peerReviews = peer,
            aggregate = aggregate,
            finalReport = o.optString("final"),
            chairmanModel = o.optString("chairman"),
            errors = errors,
            startedAt = o.optLong("started", System.currentTimeMillis()),
            finishedAt = if (o.isNull("finished")) null else o.optLong("finished"),
            verificationMemo = o.optString("verification"),
            excludedManifest = exclusions
        )
    }

    private fun decodeAudit(o: JSONObject): ModelRepoAudit {
        val batches = mutableListOf<String>()
        val ba = o.optJSONArray("batches") ?: JSONArray()
        for (i in 0 until ba.length()) batches += ba.optString(i)
        val coverage = mutableListOf<FileCoverage>()
        val ca = o.optJSONArray("coverage") ?: JSONArray()
        for (i in 0 until ca.length()) {
            val c = ca.getJSONObject(i)
            coverage += FileCoverage(c.optString("path"), c.optString("model"), c.optBoolean("covered"), c.optInt("batch"), if (c.isNull("error")) null else c.optString("error"))
        }
    ...
    private fun decodeReview(o: JSONObject): RankingReview {
        ...
        val a = o.optJSONArray("ranking") ?: JSONArray()
        for (i in 0 until a.length()) ranking += a.optString(i)
