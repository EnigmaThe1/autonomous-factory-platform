...
data class RepoFile(
    ...
    val content: String,
    val category: String,
    val binaryOrExcluded: Boolean = false,
...
data class RepoSnapshot(
    ...
    val requiredFiles: List<RepoFile> get() = files.filterNot { it.binaryOrExcluded }
}
...
data class ModelRepoAudit(
    val model: String,
    val report: String,
    val coverage: List<FileCoverage>,
    val batchReports: List<String>,
    val complete: Boolean,
    val error: String? = null
) {
    val coveredCount: Int get() = coverage.count { it.covered }
    val requiredCount: Int get() = coverage.size
}
...
data class AuditFinding(
    ...
    val disputedBy: List<String> = emptyList(),
